...
def main():
    ...
    kernel = "matern32" if args.model_type == "KRG" else "squar_exp"
    ...
    with labview.LabViewClient(dummy=(args.control_file != "")) as client:
        for i in range(args.num_steps):
            ...
            c_next, z = diff_controller.step(client)
            zs.append(z)

            if z < best_metric:
                best_metric = z
                best_metric_pt = diff_controller.control_point

            diff_controller.save_to_file(args.output)

            print(f"======= Step {i + 1} / {args.num_steps} =======")
            print("Control point: ")
            for val, name in zip(
                diff_controller.dict_to_vec(diff_controller.control_point),
